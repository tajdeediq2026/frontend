...
export default function AboutUsSection() {
  ...
  useEffect(() => {
    const fetchAboutUs = async () => {
      try {
        ...
        console.warn('Failed to load AboutUs content:', message);
  ...
  return (
    <section className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-primaryOther text-right mb-2">عن جريدة تجديد</h1>
      <div className="w-full h-1 bg-primaryOther mb-6"></div>

      {loading ? (
        <div className="bg-white rounded-lg border border-gray-200 p-6 text-center text-gray-600">جاري تحميل المحتوى...</div>
      ) : error ? (
        <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-6 text-right">{error}</div>
      ) : latestItems.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-6 text-center text-gray-600">لا يوجد محتوى متاح حالياً.</div>
      ) : (
        <div className="space-y-6">
          {latestItems.map((item) => (
            <article key={item.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 md:p-8">
              {item.title && (
                <h2 className="text-2xl font-bold text-primaryOther mb-4 text-right">{item.title}</h2>
              )}
